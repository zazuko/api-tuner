...
(async () => {
  ...
  const curlJson: Record<string, string | number | null> = Object.assign({
    '@context': {
      '@vocab': ns().value,
    },
  ...
  const contentType = typeof curlJson.content_type === 'string' && curlJson.content_type.substring(0, curlJson.content_type.indexOf(';'))
  if(contentType) {
